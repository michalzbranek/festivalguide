/**
 * @file Searches for the events according to the date.
 * @author Michal Zbranek
 */
var sPageURL = window.location.search.substring(1);
var sURLVariables = sPageURL.split('&');
var eventName, eventRadius, eventStart, eventEnd;
for (var i = 0; i < sURLVariables.length; i++) {
    ...
    if(sParameterName[0] == 'sDate'){
        eventStart = sParameterName[1];
    }
    if(sParameterName[0] == 'eDate'){
        eventEnd = sParameterName[1];
    }
}
searchEvent(eventName, eventRadius, eventStart, eventEnd);
 ...
 * @param {string} eventStart - Start of the event.
 * @param {string} eventEnd - End of the event.
 */
    function searchEvent(eventName, eventRadius, eventStart, eventEnd){
        eventName = eventName.replace('+','');
        var url = "http://api.jambase.com/events?zipCode=" + eventName + "&radius=" + eventRadius + "&startDate=" + eventStart
         + "T20%3A00%3A00&endDate=" + eventEnd + "T20%3A00%3A00&&page=0&api_key=cncesqtwe3w8gpstey3abca4";
